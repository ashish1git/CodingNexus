...
const StudentManagement = () => {
  ...
  try {
   // 1. Create email from moodleId
   const email = `${formData.moodleId}@codingnexus.com`;

      // ---------------------------------------------------------
      // START: FIX FOR CREATING AUTH USER WITHOUT LOGGING OUT ADMIN
      // ---------------------------------------------------------
      
      // Get the config from the currently running app
      const app = getApp();
      
      // Initialize a "Secondary" app instance just for this transaction
      // This prevents the main auth instance (the Admin) from being logged out
      ...
      // Get the REAL unique ID generated by Firebase Auth
      const uid = userCredential.user.uid;

      // Clean up: Sign out the new user from the secondary app immediately
      await signOut(secondaryAuth);
      // ---------------------------------------------------------
      // END: AUTH CREATION
      // ---------------------------------------------------------

   // 2. Create student document in Firestore using the REAL Auth UID
   await setDoc(doc(db, 'users', uid), {
    uid: uid, // Use the real Auth UID here
    name: formData.name,
    ...
    email: email,
        // Security Note: Storing password in DB is risky, but kept as per your original code
    password: formData.password, 
    role: 'student',
    attendance: 0,
    photoURL: null,
        // Set this to FALSE now, because we just created the Auth account!
   ...
   console.error('Error adding student:', error);
      ...
      if (error.code === 'auth/email-already-in-use') {
        errorMessage = 'A student with this Moodle ID/Email already exists in Authentication.';
      }
      
   toast.error('Failed to add student: ' + errorMessage);
  } finally {
      // Vital: Delete the secondary app to free up resources
      if (secondaryApp) {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 mb-4 sm:mb-6">
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-4 mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-200">
            <div className="text-center">
              <p className="text-xl sm:text-2xl font-bold text-gray-800">{filteredStudents.length}</p>
              <p className="text-xs sm:text-sm text-gray-600">Total</p>
            </div>
            <div className="text-center">
              <p className="text-xl sm:text-2xl font-bold text-blue-600">
                {filteredStudents.filter(s => s.batch === 'Basic').length}
              </p>
              <p className="text-xs sm:text-sm text-gray-600">Basic</p>
            </div>
            <div className="text-center">
              <p className="text-xl sm:text-2xl font-bold text-purple-600">
                {filteredStudents.filter(s => s.batch === 'Advanced').length}
              </p>
            ...
            <form onSubmit={handleAddStudent} className="space-y-4">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <p className="text-xs text-blue-800">
                  ℹ️ Student will be created in database. They will create their Firebase account on first login.
                </p>
              </div>
