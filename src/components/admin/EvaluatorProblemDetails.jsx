...
import { ChevronDown, ChevronUp, FileText, CheckCircle2, XCircle, Eye, EyeOff, Braces, GitBranch } from 'lucide-react';
...
export const TestResultsBreakdown = ({ testResults }) => {
    ...
    <div className="mt-3">
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs font-bold text-gray-700 flex items-center gap-1.5">
          <GitBranch className="w-3.5 h-3.5 text-gray-400" /> Test Case Results
        </p>
...
const EvaluatorProblemDetails = ({ problem, submission }) => {
  const [open, setOpen] = useState(false);
  const problemData = problem || submission?.problem;
  const testResults = submission?.testResults;
  const hasTestResults = Array.isArray(testResults) && testResults.length > 0;
  const hasContent = problemData?.description || problemData?.examples?.length || problemData?.constraints?.length;
        ...
        <span className="flex items-center gap-2 text-sm font-semibold text-gray-800">
          ...
          {hasTestResults && (
            <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700">
              {submission?.testsPassed ?? testResults.filter(t => t.passed).length}/{submission?.totalTests ?? testResults.length} passed
            </span>
          )}
        </span>
        ...
        <div className="p-4 sm:p-5 space-y-5">
          ...
          {problemData?.description && (
            <div>
              ...
              <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line break-words">{problemData.description}</p>
            </div>
          )}
          ...
          {/* Constraints */}
          {Array.isArray(problemData?.constraints) && problemData.constraints.length > 0 && (
            <div>
          ...
          {hasTestResults && <TestResultsBreakdown testResults={testResults} />}
        </div>
      )}
    </div>
  );
...
export default EvaluatorProblemDetails;
