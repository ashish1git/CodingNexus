#!/usr/bin/env node
/**
 * Send Shortlist Emails via Server API
 * Fetches applicants from running server and sends emails
 */
...
const API_URL = process.env.API_URL || 'http://localhost:22000/api';
...
async function fetchApplicantsFromPublicAPI() {
  try {
    console.log('\n📧 Sending Shortlist Emails via Server API');
    console.log('═'.repeat(75) + '\n');

    console.log('🔍 Fetching applicants from server...\n');
    ...
    // Filter out those without proper names/emails
    const validApplicants = applicants.filter(app => app.fullName && app.email);

    if (validApplicants.length === 0) {
      console.log('❌ No valid applicants (missing name or email).\n');
      rl.close();
      process.exit(0);
    }

    // Display list
    console.log(`✅ Found ${validApplicants.length} applicants:\n`);
    console.log('─'.repeat(75));
    validApplicants.forEach((app, index) => {
    ...
    // Confirm
    const confirm = await question(`⚠️  Ready to send emails to ${validApplicants.length} applicants? (yes/no): `);
    ...
    console.log(`📊 Total applicants: ${validApplicants.length}`);
    if (successCount > 0) {
      console.log(`📈 Success Rate: ${Math.round((successCount / validApplicants.length) * 100)}%`);
    }
    console.log('═'.repeat(75) + '\n');
    ...
    console.error('\n❌ ERROR:', error.message);
    console.error('\nTroubleshooting:');
    console.error('1. Make sure the server is running on port 22000');
    console.error('2. Check that team applications exist in the database');
    console.error('3. Ensure the API endpoint is accessible\n');
