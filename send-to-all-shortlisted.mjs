#!/usr/bin/env node
/**
 * Send Interview Shortlist Email to ALL Applicants
 * Personalizes with actual names from database
 * Shows list before sending
 */
...
async function sendToAll() {
  try {
    ...
    console.log(`✅ Found ${applicants.length} shortlisted applicants:\n`);
    console.log('─'.repeat(70));
    ...
    for (const applicant of applicants) {
      try {
        ...
        if (result.success) {
          ...
          console.log(`❌ ${applicant.fullName.padEnd(30)} ← ${applicant.email} (Error: ${result.error})`);
        }
      } catch (error) {
        failureCount++;
        errors.push({ email: applicant.email, name: applicant.fullName, error: error.message });
        console.log(`❌ ${applicant.fullName.padEnd(30)} ← ${applicant.email} (Error: ${error.message})`);
      }
    }
