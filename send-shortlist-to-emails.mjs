#!/usr/bin/env node
/**
 * Send Interview Shortlist Email - Manual Mode
 * You provide the list of emails and names
 */
...
async function sendToAll() {
  try {
    console.log('\n📧 Interview Shortlist Email - Send to Multiple Applicants');
    console.log('═'.repeat(70) + '\n');

    // Get list of emails from user
    const emailsInput = await question('📧 Enter list of emails (comma-separated):\n   Example: name1@email.com, name2@email.com\n\n> ');
    ...
    console.log(`\n✅ Got ${emails.length} email(s)\n`);

    // Get names for each email or use generic names
    const applicants = [];
    for (let i = 0; i < emails.length; i++) {
      const name = await question(`Enter full name for ${emails[i]}: `);
      applicants.push({
        email: emails[i],
        fullName: name || `Applicant ${i + 1}`
      });
    }

    // Display list
    console.log('\n─'.repeat(70));
    console.log('SHORTLIST APPLICANTS:');
    console.log('─'.repeat(70));
    applicants.forEach((app, index) => {
      console.log(`${index + 1}. ${app.fullName.padEnd(30)} | ${app.email}`);
    });
    console.log('─'.repeat(70) + '\n');
    ...
    for (const applicant of applicants) {
      try {
        const result = await sendEmail({
          ...
          html: emailTemplate(applicant.fullName),
        ...
        if (result.success) {
          successCount++;
          console.log(`✅ ${successCount}. ${applicant.fullName.padEnd(30)} ← ${applicant.email}`);
        } else {
          failureCount++;
          errors.push({ email: applicant.email, name: applicant.fullName, error: result.error });
          console.log(`❌ ${applicant.fullName.padEnd(30)} ← ${applicant.email}`);
        }
      } catch (error) {
        failureCount++;
        errors.push({ email: applicant.email, name: applicant.fullName, error: error.message });
        console.log(`❌ ${applicant.fullName.padEnd(30)} ← ${applicant.email}`);
      }
    }
