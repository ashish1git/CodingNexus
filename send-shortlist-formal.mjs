#!/usr/bin/env node
/**
 * Send Interview Shortlist Email - Formal & Short
 */

import 'dotenv/config';
import { sendEmail } from './server/services/email/brevo.service.js';

const formalEmailTemplate = (applicantName) => {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
...
async function sendShortlistEmail() {
  try {
    console.log('\n📧 Interview Shortlist Email - Formal & Short');
    ...
    const result = await sendEmail({
      ...
      html: formalEmailTemplate(applicantName),
    ...
    if (result.success) {
      ...
      console.log('✨ Clean, formal, short template delivered!\n');
    } else {
      console.log('❌ FAILED!');
